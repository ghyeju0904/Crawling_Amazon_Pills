...
# 검색 버튼 XPath.
SEARCH_BUTTON_XPATH = "/html/body/div[1]/header/div/div[1]/div[2]/div/form/div[3]/div/span/input" # ⭐id="nav-search-submit-button" 으로 써도 되는지지
...
def extract_products_on_page(driver): # on_page -> 현재 페이지에서,  extract_products -> 모든 상품 정보 추출, driver(브라우저 조종하는 셀레니움 객체)를 받아서 실행
    ...
    print(f"[DEBUG] cards: {len(cards)}")  # 찾은 상품 카드 수 출력

    results = []  # 결과 담을 빈 리스트
    for card in cards:
        try:
            title = card.find_element(By.CSS_SELECTOR, "h2 span").text.strip()  # 상품 제목 찾기
        except:
            try:
                title = card.find_element(By.CSS_SELECTOR, "h2 a span").text.strip()
            except:
                title = "상품명 없음"  # 못 찾으면 기본값
        ...
        print(f"[DEBUG] {title} | {price} | {image}")
        results.append({
            "title": title,
            "price": price,
            "image_url": image
...
def crawl_by_search(query, max_pages=10):
    ...
    try:
        ...
        while True:
            ...
            print(f"[INFO] {page}페이지 상품 {len(items)}개")
            all_items.extend(items)  # 결과 누적 저장
            if page >= max_pages:  # 최대 페이지 도달 시 멈춤
                break
            moved = _click_next(driver)  # 다음 페이지로 이동 시도
            if not moved:
                break
            page += 1
    finally:
        driver.quit()  # 작업 끝나면 브라우저 닫기
    return all_items  # 전체 결과 반환
